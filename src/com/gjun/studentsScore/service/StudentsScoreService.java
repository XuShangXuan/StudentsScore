...
public class StudentsScoreService {
	...
	public StudentsReport inputStudentsScoreData(Path filePath) throws IOException {
		...
		for (int i = 1; i < lines.size(); i++) {
			...
			for (int j = 2; j < column.length; j++) { // 從第三欄開始會是各科成績
				...
				sum += Integer.parseInt(column[j]);// 加總各科成績

			}

			double ave = (double) sum / (column.length - 2);// 總成績/總科目數得到平均成績
			ave = Math.round(ave * 10.0) / 10.0;// 平均成績算到小數點後第一位

			stu.setScore(scoreData);// 將成績存入StudentsScore的"成績"欄位中
			stu.setSumScore(String.valueOf(sum));// 將總成績存入StudentsScore的"總成績"欄位中
			stu.setAverage(String.valueOf(ave));// 將平均成績存入StudentsScore的"平均成績"欄位中
	...
	public void outputStudentsScoreData(Path targetPath, StudentsReport studentsReport) throws IOException {
		...
		for (int i = 0; i < studentsReport.getStudentsScoreData().size(); i++) {
			...
			// 串入各科成績
			for (int j = 0; j < studentsReport.getStudentsScoreData().get(i).getScore().size(); j++) {
				line.append(studentsReport.getStudentsScoreData().get(i).getScore().get(j));
				line.append(",");
			}
